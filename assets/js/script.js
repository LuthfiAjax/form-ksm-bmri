function populateCabangSelect() {
	const baseurl = "http://localhost/api-form-bmri/";
							...
							success: function (data) {
								if (data && Array.isArray(data)) {
									data.forEach((kelolaan) => {
										const option = $("<option></option>")
											.val(kelolaan.NAMA_K_L)
											.text(kelolaan.NAMA_K_L);
										...
										$.ajax({
											...
											success: function (data) {
												...
												if (data && Array.isArray(data)) {
													data.forEach((satker) => {
														const option = $("<option></option>")
															.val(satker.NAMA_NASABAH)
															.text(satker.NAMA_NASABAH)
															.data("count-cif", satker.COUNT_CIF)
															.data("indikatif", satker.INDIKATIF);
													...
													satkerSelect.on("change", function () {
														...
														const vleadscabang =
															selectedOption.data("count-cif");
														const vindikatifcabang =
															selectedOption.data("indikatif");
